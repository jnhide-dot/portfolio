...
main.innerHTML='<section class="showcase-hero"><div class="hero-panels">'+images.map((i,n)=>'<a class="hero-panel" href="'+E(i.homepage)+'" target="_blank" rel="noopener noreferrer" style="--image-position:'+E(i.position||'50%')+'">'+imageTag(i,'',n===0)+'<span class="hero-game-name">'+E(i.title)+'<b>↗</b></span></a>').join('')+'</div><div class="hero-heading"><p>JNHIDE / GAME DESIGN</p><h1>오타쿠의 상상이<br>현실로</h1><p class="hero-subtitle">전투 시스템·캐릭터 기획 · 어 식</p></div><div class="hero-bottom"><span>분석에서 기획으로</span><a href="#/projects">프로젝트 보기 <b>→</b></a><a href="#/playlist">플레이리스트 <b>→</b></a></div></section>'+profileSection()+'<section class="home-section"><div class="section-title"><div><p class="eyebrow">PORTFOLIO</p><h2>포트폴리오</h2></div><a href="#/portfolio">전체 보기 ↗</a></div>'+spiritCard()+'</section><section class="home-section"><div class="section-title"><div><p class="eyebrow">PROJECTS</p><h2>프로젝트 기록</h2></div><a href="#/projects">전체 보기 ↗</a></div>'+projects()+'</section><section class="playlist-banner"><div><p class="eyebrow">PLAYLIST</p><h2>플레이리스트</h2><p>'+playlist.length+'개의 게임, 플레이 기록과 전투 경험.</p><a class="solid-link" href="#/playlist">플레이리스트 보기 →</a></div><div class="playlist-name-wall">'+playlist.slice(0,12).map(g=>'<a href="#/games/'+g.id+'">'+E(g.title)+'</a>').join('')+'</div></section>';
}
